...
FROM AGENT_AUDIT.OBSERVABILITY.AGENT_TOOL_USAGE
WHERE EVENT_DATE >= DATEADD('day', -30, CURRENT_DATE())
...
FROM AGENT_AUDIT.OBSERVABILITY.FAILED_QUERIES_ANALYSIS
...
SELECT 
    ...
    start_time
FROM AGENT_AUDIT.OBSERVABILITY.AGENT_QUERY_HISTORY
...
FROM AGENT_AUDIT.OBSERVABILITY.CORTEX_ANALYST_REQUESTS_FLATTENED
...
FROM AGENT_AUDIT.OBSERVABILITY.CORTEX_ANALYST_SEARCH_USAGE
...
FROM AGENT_AUDIT.OBSERVABILITY.AGENT_ANALYST_CORRELATION
...
FROM AGENT_AUDIT.OBSERVABILITY.AGENT_EVENTS_FLATTENED ae
...
BEGIN
    ...
    SELECT * FROM AGENT_AUDIT.OBSERVABILITY.USER_ACTIVITY_SUMMARY 
    WHERE user_name = :target_user 
    ORDER BY activity_date DESC;
    
    -- User's failed queries
    SELECT * FROM AGENT_AUDIT.OBSERVABILITY.FAILED_QUERIES_ANALYSIS 
    WHERE user_name = :target_user 
