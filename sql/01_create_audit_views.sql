...
- AI_OBSERVABILITY_EVENTS and CORTEX_ANALYST_REQUESTS_RAW populated with events

================================================================================
VIEWS CREATED BY THIS SCRIPT
================================================================================

AGENT VIEWS (from AI_OBSERVABILITY_EVENTS):
1.  AGENT_EVENTS_FLATTENED       - Base flattened view of all agent events
2.  AGENT_CONVERSATIONS          - User queries and agent responses
3.  AGENT_TOOL_USAGE             - Tool invocations by agents
4.  AGENT_FEEDBACK_SUMMARY       - Daily feedback aggregation

INFRASTRUCTURE VIEWS (from ACCOUNT_USAGE):
5.  AGENT_QUERY_HISTORY          - Cortex-related query history
6.  DATA_ACCESS_LINEAGE          - Tables/columns accessed
7.  FAILED_QUERIES_ANALYSIS      - Failed query categorization
8.  USER_ACTIVITY_SUMMARY        - Daily user activity metrics

CORTEX ANALYST VIEWS (from CORTEX_ANALYST_REQUESTS_RAW):
9.  CORTEX_ANALYST_REQUESTS_FLATTENED  - Base flattened view of analyst requests
10. CORTEX_ANALYST_USAGE_SUMMARY       - Daily usage by model/user
11. CORTEX_ANALYST_PERFORMANCE         - Hourly performance metrics
12. CORTEX_ANALYST_VERIFIED_QUERY_USAGE - Verified query match tracking
13. CORTEX_ANALYST_SEARCH_USAGE        - Cortex Search calls within Analyst

COMBINED VIEWS:
14. AGENT_ANALYST_CORRELATION    - Correlates agent events with analyst requests

================================================================================
SCHEMA REFERENCE
================================================================================

AI_OBSERVABILITY_EVENTS (actual column names from DESCRIBE):
------------------------------------------------------------
...
- TRACE                   : Tracing context (OBJECT) - contains trace_id, span_id
- RESOURCE                : For future use (OBJECT)
- RESOURCE_ATTRIBUTES     : Source identification (OBJECT) - contains:
    * "snow.user.name", "snow.user.id"
    * "snow.session.id", "snow.session.role.primary.name"
- SCOPE                   : Scope for signals (OBJECT) - contains:
    * "name" (e.g., "snow.cortex.agent")
- SCOPE_ATTRIBUTES        : For future use (OBJECT)
- RECORD_TYPE             : Type of RECORD value (VARCHAR) - e.g., "SPAN"
- RECORD                  : Fixed fields per signal type (OBJECT) - contains:
    * "name" (e.g., "Agent")
    * "kind" (e.g., "SPAN_KIND_INTERNAL")
    * "status" -> "code" (e.g., "STATUS_CODE_OK")
- RECORD_ATTRIBUTES       : Variable attributes (OBJECT) - contains:
...
----------------------------
(Run DESCRIBE TABLE to confirm - assumed similar to AI_OBSERVABILITY_EVENTS)
- TIMESTAMP               : Request timestamp
- RESOURCE_ATTRIBUTES     : Variant with semantic model & user info:
    * "snow.user.name", "snow.user.id"
    * "snow.session.id", "snow.session.role.primary.name"
    * "snow.semantic_model.name" (e.g., "@DB.SCHEMA.STAGE/model.yaml")
...
- RECORD                  : Variant with event name:
    * "name" (e.g., "CORTEX_ANALYST_REQUEST")
- RECORD_ATTRIBUTES       : Variant with RICH CONTENT:
    * "request_id"
    * "llm.app.type" (CORTEX_ANALYST)
    ...
    * "response_body" -> "message" -> "content"[]:
        - [0]: text interpretation
        - [1]: sql statement + confidence + verified_query_used
    * "response_body" -> "response_metadata":
        - "question_category" (e.g., "CLEAR_SQL")
        - "model_names" (e.g., ["claude-3-5-sonnet"])
        - "cortex_search_retrieval" (array of search results)
